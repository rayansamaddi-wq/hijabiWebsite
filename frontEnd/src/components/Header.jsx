...
const Header = () => {
  ...
  const { cartItems, cartAdded } = useSelector((state) => state.cart);
...
>
  ...
  {cartItems.length > 0 && (
    <span className='flex h-5 min-w-5 items-center justify-center rounded-full bg-[#d9b8ae] px-1 text-[10px] text-white'>
      {cartItems.reduce((total, item) => total + item.qty, 0)}
    </span>
  )}

  {cartAdded && (
    <span className='absolute -right-5 -top-5 animate-cart-added text-sm font-bold text-red-500'>
      +1
    </span>
  )}
</Link>
